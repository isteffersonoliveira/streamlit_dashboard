...
def converte_excel(df):
    # Cria um objeto BytesIO para armazenar os dados do arquivo Excel
    buffer = io.BytesIO()
...
with coluna1:
    ...
    nome_arquivo_xls = st.text_input('', label_visibility='collapsed', value='dados')
    nome_arquivo_xls += '.xlsx'

    #nome_arquivo_csv = st.text_input('', label_visibility='collapsed', value='dados')
    #nome_arquivo_csv += '.csv'

with coluna2:

    button = st.download_button('Fazer o download da tabela em Excel', data=converte_excel(dados_filtrados),
                                file_name=nome_arquivo_xls,
                                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                on_click=mensagem_sucesso)

    # st.download_button('Fazer o download da tabela em Excel', data=converte_csv(dados_filtrados), file_name=nome_arquivo_csv, mime='text/csv', on_click=mensagem_sucesso)